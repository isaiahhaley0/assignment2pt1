function searchGoogleBooks(){
    ...
    if(txtSearch.value==""){
        ...
        document.querySelector("#mainDiv").innerHTML = "Nosearch keyword provided...";
        return;
    }
    ...
    .then(json =>{
    var list = `<div class ="row col-sm-5">found ${json.totalItems} books</div> <div class="row">`;
        ...
        for(let i = 0; i < json.items.length; i++){
            ...
            template += `<a class="list-group-item list-group-ittem-action active" href ="${json.items[i].selfLink}">${bookInfo.title}</a>` 
            template += `<li class="list-group-item">${bookInfo.authors}</li>` 
            
            template += `<a class="list-group-item">${bookInfo.publisher},${bookInfo.publishedDate}</li>` 
            template += `</ul>` 
            list += template;
